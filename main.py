...
import nonebot_plugin_localstore as fileStorage
...
helpCmd = on_command("#help", rule=checkIfListenpro)
game = on_command("game", rule=checkIfListenpro)
unlock = on_command("unlock", rule=checkIfListenpro)
rank = on_command("rank", rule=checkIfListenpro)
trank = on_command("trank", rule=checkIfListenpro)
q = on_command("q", rule=checkIfListenpro)
team = on_command("team", rule=checkIfListenpro)
open = on_command("#open", rule=checkIfListenpro)
close = on_command("close", rule=checkIfListenpro)
openb = on_command("openb", rule=checkIfListenpro)
closeb = on_command("closeb", rule=checkIfListenpro)
qa = on_command("qa", rule=checkIfListenpro)
ban = on_command("ban", rule=checkIfListenpro)
resetpwd = on_command("resetpwd", rule=checkIfListenpro)
openq = on_command("openq", rule=checkIfListenpro)
closeq = on_command("closeq", rule=checkIfListenpro)
addnotice = on_command("addnotice", rule=checkIfListenpro)
addhint = on_command("addhint", rule=checkIfListenpro)

bind = on_command("#bind", rule=checkIfListenOrPrivate)

bindpro = on_command("#bindpro", rule=checkIfListenpro)

from nonebot import on_keyword


# banyty = on_command("",rule=isYTY)
# @banyty.handle()

# async def banyty(bot,event:MessageEvent,args: Message = CommandArg()):

#     await bot.send(event,f'我嘞个豆，居然是宇宙无敌帅天宇哥哥出来了')



@bindpro.handle()

async def bindUserForQQPro(bot,event:MessageEvent,args: Message = CommandArg()):

    # qqnumber = str(event.user_id) # qq
    global GAME_LIST
    userName = args.extract_plain_text() # userName
    tempdata = (userName.split(" "))
    # gameList = getGameList() #比赛列表

    for temp in GAME_LIST:

        gameId = temp['id'] # 比赛id

    filepath = "data"+str(gameId)+".txt"
    ProFile = fileStorage.get_plugin_data_file(filepath)
    # fileStorageData = ProFile.read_text()
    qqnumberPro = tempdata[0]
    userNamePro = tempdata[1]
    # print(qqnumberPro)
    # print(teamNamePro)
    gameNamePro = getGameInfo(gameId)['title']
    gameIdTempPro = getTeamInfoWithGameId(gameId)
    # getTeamInfoWithGameId
    # getTeamInfoWithName()
    # if gameIdPro:
        # print(gameIdPro)
    # print(gameIdTempPro)
    # gameIdPro = gameIdTempPro['id']
    for tempgameIdPro in gameIdTempPro:
        if userNamePro == tempgameIdPro['team']['members'][0]['userName']:
            gameIdPro = tempgameIdPro['id']    
            teamNamePro = tempgameIdPro['team']['name']
            with ProFile.open('a') as f:
                f.write(str(qqnumberPro)+"##分割字符##"+str(gameIdPro)+"##分割字符##"+str(teamNamePro)+"\n")
            data = f'用户【{userNamePro}】所对应的队伍【{teamNamePro}】已经成功绑定QQ【{qqnumberPro}】，其在比赛【{gameNamePro}】中的ID为【{gameIdPro}】,请自行核实正确性'
            break
        else:
            # pass
            data = '请核实用户名【{userNamePro}】是否正确，或联系黑奴D0G解决'
            # break
    await bot.send(event,data)

@bind.handle()

async def bindUserForQQ(bot,event:MessageEvent,args: Message = CommandArg()):
    ...
    filepath = "data"+str(gameId)+".txt"
    tempFile = fileStorage.get_plugin_data_file(filepath)
    fileStorageData = tempFile.read_text()
    allQQId = []
    AllGameId = []
    AllGameName = []
    with tempFile.open('r') as tempFileDataApi:
        for tempFileData in tempFileDataApi:
            tempdata = (tempFileData.strip().split('##分割字符##'))
            print(tempdata)
            allQQId.append(tempdata[0])
            AllGameId.append(tempdata[1])
            AllGameName.append(tempdata[2])


    hasacc = f'用户【{userName}】已经审核&绑定成功'
    noteam = f'用户【{userName}】没有报名&绑定失败'
    accsuc = f'用户【{userName}】审核&绑定成功'
    # hasbind = f'【{qqnumber}】已经绑定队伍{}，如有疑问请联系管理员'
    teamhasbind = f'平台用户【{userName}】已经被绑定，如有疑问请联系管理员'
    error = '出现错误，请联系管理员'
    if userName:
        
            if qqnumber not in allQQId:

                '''
                    判断此QQ是否已经绑定 team 
                    绑定 -> 已经绑定
                    未绑定 -> 继续进行判断
                    增加一个绑定team的自助查询功能【todo】
                '''

                for team in getTeamInfoWithGameId(gameId):

                    '''
                        获取比赛报名的所有队伍信息
                    '''

                    # print(team['team']['members'][0]['userName'])

                    # print(type(team['team']['members'][0]['userName']))

                    

                    if team['team']['members'][0]['userName'] == userName:
                            ...
                            '''
                                绑定发起人未绑定的情况下，判断要绑定的队伍是否被绑定
                                已经绑定--》直接发送已经被绑定
                                未绑定--〉进下面判断
                            '''
                            teamFlag = str(team['id'])
                            if teamFlag not in AllGameId:
                                    ...
                                    if team['status'] == 'Accepted':
                                        ...
                                        # with tempFile.open('a') as f:
                                        #     f.write(qqnumber+"##"+str(team['id'])+"\n")
                                    
                                        # cache_file = fileStorage.get_plugin_cache_file("file_name")

                                        # print(fileStorageData)
                                        # print(cache_file)
                                        temp = hasacc
                                        break

                                    if AcceptedTeam(team['id']):

                                        '''
                                            进入到未审核状态
                                            写入绑定信息 QQ##teamId##队伍名称
                                        '''
                                        with tempFile.open('a') as f:
                                            f.write(qqnumber+"##分割字符##"+str(team['id']+"##分割字符##"+team['team']['name'])+"\n")
                                ...
                                # with tempFile.open('r') as tempFileDataApi:
                                    # for tempFileData in tempFileDataApi:
                                        # tempdata = (tempFileData.strip().split('##'))
                                # teamhasbind = f'平台用户【{team[]}】已经被绑定，如有疑问请联系管理员'
                                temp = teamhasbind
                                break
                ...
                with tempFile.open('r') as tempFileDataApi:
                    for tempFileData in tempFileDataApi:
                        tempdata = (tempFileData.strip().split('##分割字符##'))
                        if qqnumber == tempdata[0]:
                            teamName = tempdata[2]

                hasbind = f'【{qqnumber}】已经绑定队伍【{teamName}】，如有疑问请联系管理员'
                await bot.send(event,hasbind)
    else:
        await bot.send(event,"请输入平台注册用户名，例如：#bind cyyz")
